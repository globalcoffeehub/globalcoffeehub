! function (t) {
    ...
    L.exportLayer = L.Class.extend({
        ...
        _init: function () {
            ...
            if (t.style.position = "absolute", t.style.width = this._map.getSize().x + "px", t.style.height = this._map.getSize().y + "px", this.container = t, this.canvas = document.createElement("canvas"), this.context = this.canvas.getContext("2d"), t.appendChild(this.canvas), this._map.getPanes().overlayPane.appendChild(t), !this.export) {
        ...
        _draw: function () {
            var t = this._map.getBounds()
            if (t && this.export.playAnimation) {
                this._resize(), this._transform()
                var i = this._convertData()
                this.export.updateData(i), this.export.start(this.canvas)
            }
        },
        _transform: function () {
            var t = this._map.getBounds(),
        ...
        destroy: function () {
            this.export.clear(), this.container.parentNode.removeChild(this.container), this._map.clearAllEventListeners(), this.mapHandles = []
        }
    }), L.exportLayer = function (t) {
        return new L.exportLayer(t)
    }
}(window)